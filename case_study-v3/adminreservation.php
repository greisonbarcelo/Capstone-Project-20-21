...
<div class="container-xl" data-aos="fade-up">
    <div class="table-responsive">
        <div class="table-wrapper">
            <div class="table-title">
                <div class="row">
                    <div class="col-sm-8 col-lg-9"><h1>Reservation Details </h1></div>
                    <div class="col-sm-4 col-lg-3">
                        <a href="createreservation.php" class="btn btn-get-started">
                            Create Reservation
                        </a>
                    </div>        
	</div>
            ...
            <table class="table table-striped table-hover table-bordered">
                ...
                <?php 
                    ...
                    while ($row = mysqli_fetch_assoc($select)) {
                            ...
                            echo "<td class='td'>" . $row['entrance_type']."</td>";
                            echo "<td class='td'>" . $row['room_type']."</td>";
                            echo "<td class='td'>". $row['people'] . '</td>';
                            echo "<td class='td'>". $row['adult'] . '</td>';
                            echo "<td class='td'>". $row['id'] . '</td>';
                            echo "<td class='td'>". $row['senior'] . '</td>';
                            echo "<td class='td'>". $row['email'] . '</td>';                    
                            echo "<td class='td'>". $row['payment'] . '</td>';
                            
                            echo " 
                            <td>
                                <a href='adminreservation.php?view=$row[id]' class='view' title='View' data-toggle='tooltip'><i class='material-icons'>&#xE417;</i></a>
                                <a href='adminreservation.php?edit=$row[id]' class='edit' title='Edit' data-toggle='tooltip'><i class='material-icons'>&#xE254;</i></a>
                                <a href='adminreservation.php?del=$row[id]' class='delete' title='Delete' data-toggle='tooltip'><i class='material-icons'>&#xE872;</i></a>
                            </td>
                            ";
                            echo "</tr>";
                    }
            ...
            <?php
                if (isset($_GET['del'])) {
                    $del = $_GET['del'];  
                    $delete = mysqli_query($con, "DELETE FROM tbl_reservation WHERE id = '$del'");

                    echo "<script>alert('Successfully Deleted an Reservation.');window.location.href='adminreservation.php'</script>";
                }
            ?>
            <?php 
                if (($_GET['view'])) {
                    $view = $_GET['view'];
                    $select = mysqli_query($con, "SELECT * FROM tbl_reservation WHERE id = '$view'");
                    while ($row = mysqli_fetch_assoc($select)) {
                        $id = $row['id'];
                        $date = $row['date'];
    ...
    <div class="modal-dialog modal-login">
			<div class="modal-content">
				...
				<div class="modal-body">
						...
						<div class="form-group">
							<?php
                                echo "Reservation Date: <br>". $date;
                            ?>		
						</div>

                        <div class="form-group">
							<?php
                                echo "Reservation ID: <br>". $id;
                            ?>		
						</div>
                        <div class="form-group">
							<?php
                                echo "Entrance Type: <br>". $entrance_type;
                            ?>		
						</div>
                        <div class="form-group">
							<?php
                                echo "Room Type: <br>". $room_type;
                            ?>		
						</div>
                        <div class="form-group">
							<?php
                                echo "People: <br>". $people;
                            ?>		
						</div>
                        <div class="form-group">
							<?php
    ...
    <?php
        ...
        elseif (($_GET['edit'])) {
            $edit = $_GET['edit'];
            $select = mysqli_query($con, "SELECT * FROM tbl_reservation WHERE id = '$edit'");
            while ($row = mysqli_fetch_assoc($select)) {
                $fname = $row['fname'];
                $lname = $row['lname'];
                $id = $row['id'];
     ...
     <div class="modal-dialog modal-login">
			<div class="modal-content">
				<div class="modal-header">
					...
					<h4 class="modal-title">Review this Reservation</h4>	
                	<button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
				</div>
				<div class="modal-body">
                ...
                <div class="modal-body">
                    ...
                    <form class="form" method="POST" enctype="multipart/form-data">
                    <div class="form-group">
                        <button class="btn-primary" type="submit" name="approve">Approve</button>
                        <button class="btn-danger"  type="submit" name="reject">Reject</button>			
                    </div>
                    </form>
            </div>	
			</div>
        ...
        <?php
            if (isset($_POST['approve'])) {
                ...
                $select = mysqli_query($con, "SELECT * FROM tbl_reservation WHERE id = '$edit'");
                    while($rows = mysqli_fetch_assoc($select)){
                        $email = $rows['email'];
                        $update = mysqli_query($con, "UPDATE tbl_reservation SET payment='$payment' WHERE id = '$edit'");
                        $update = mysqli_query($con, "UPDATE tbl_user SET reservation='$payment' WHERE email = '$email'");
                        if (!$update) {
                            die("unable to save!");
                        }
                        else{
                            echo "<script>alert('Successfully Approved Reservation.');window.location.href='adminreservation.php'</script>";
                        }
                    }
            }
                ...
                $select = mysqli_query($con, "SELECT * FROM tbl_reservation WHERE id = '$edit'");
                    while($rows = mysqli_fetch_assoc($select)){
                        ...
                        if (!$update) {
                            ...
                            echo "<script>alert('Successfully Rejected Reservation.');window.location.href='adminreservation.php'</script>";
                        }
                    }
            }
        ?>
