...
<!-- ======= Hero Section ======= -->
<section id="hero" class="herooo d-flex align-items-center">
<div class="container" data-aos="zoom-out" data-aos-delay="200" >
            ...
            <div class="row">
                ...
                <div class="col-xs-6 col-sm-6 col-md-6 text-right">
                    <p>
                        <em>Date of Payment:
                            <?php 
                                $select = mysqli_query($con, "SELECT * FROM tbl_payment WHERE email = '$_SESSION[email]'");
                                $rows = mysqli_fetch_assoc($select);
                                $payment_date = $rows['payment_date'];
                                $payment_time = $rows['payment_time'];
                                echo $payment_date . " " . $payment_time;
                            ?>
                        </em>
                    </p>
                    <p>
        ...
        <div class="container mt-5 d-flex justify-content-center">
            <div class="card p-4">
                <div class="d-flex justify-content-between align-items-center">
                        <h5 class="total-amount pt-2" style="color: black;">Amount Paid</h5>
            ...
            <div>
                <label class="d-flex justify-content-between">
                    <span class="label-text label-text-cc-number">Payment ID</span>
                    </label> 
                    <input type="tel" name="gcash_number" class="form-control credit-card-number" maxlength="11" readonly placeholder="
                        <?php 
                            $select = mysqli_query($con, "SELECT * FROM tbl_payment WHERE email = '$_SESSION[email]'");
                            $rows = mysqli_fetch_assoc($select);
                            $id = $rows['id'];
            ...
            <div>
                <label class="d-flex justify-content-between">
                    <span class="label-text label-text-cc-number pt-4">Screenshot of Payment</span>
                </label> 
                    <?php
                        $select = mysqli_query($con, "SELECT * FROM tbl_payment WHERE email = '$_SESSION[email]'");
                        while($rows = mysqli_fetch_assoc($select)){
                ...
                <?php
                    ...
                    while($rows = mysqli_fetch_assoc($select)){
                        echo "<a href = 'status.php?del=$rows[id]'>
                            <button type='button' class='btn btn-lg cancel-btn'>
                                Cancel Reservation
                            </button>
                          </a>";
                    }
    ...
    <?php
        if (isset($_GET['del'])) {
            ...
            if ($reservation == "pending_paid" || $reservation == "approved") {
                $id = $_GET['del'];
                $delete = mysqli_query($con, "DELETE FROM tbl_payment WHERE email = '$_SESSION[email]'");
                $delete = mysqli_query($con, "DELETE FROM tbl_reservation WHERE email = '$_SESSION[email]'");

                $reservation = "pending_refund";
            ...
            } elseif ($reservation == "pending_unpaid") {
                $id = $_GET['del'];
                $delete = mysqli_query($con, "DELETE FROM tbl_payment WHERE email = '$_SESSION[email]'");
                $delete = mysqli_query($con, "DELETE FROM tbl_reservation WHERE email = '$_SESSION[email]'");
            ...
            //     $delete = mysqli_query($con, "DELETE FROM tbl_reservation WHERE email = '$id'");
            //     $delete = mysqli_query($con, "DELETE FROM tbl_payment WHERE email = '$id'");
            // }
            // 
        }
    ?>
