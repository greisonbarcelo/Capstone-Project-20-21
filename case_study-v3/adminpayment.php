    ...
    <div class="table-responsive">
        <div class="table-wrapper">
            <div class="table-title">
                <div class="row">
                    <div class="col-sm-8 col-lg-9"><h1>Payment Details </h1></div>
            ...
            <table class="table table-striped table-hover table-bordered">
                ...
                <?php 
                    $select = mysqli_query($con, "SELECT * FROM tbl_payment");
                    while ($row = mysqli_fetch_assoc($select)) {
                            echo "<tr>";
                            echo "<td class='td'>". $row['id'] . '</td>';
                            echo "<td class='td'>". $row['amount_paid'] .'</td>';
                            echo "<td class='td'>" . $row['gcash_number']."</td>";
                            echo "<td class='td'>" . $row['reference_no']."</td>";
                            echo "<td class='td'>". $row['reservation_id'] . '</td>';
                            echo "<td class='td'>". $row['email'] . '</td>';
                            echo "<td class='td'>". $row['payment_date'] . '</td>';
                            echo "<td class='td'>". $row['payment_time'] . '</td>';
                            
                            echo " 
                            <td>
                                <a href='adminpayment.php?view=$row[id]' class='view' title='View' data-toggle='tooltip'><i class='material-icons'>&#xE417;</i></a>
                                
                                <a href='adminpayment.php?del=$row[id]' class='delete' title='Delete' data-toggle='tooltip'><i class='material-icons'>&#xE872;</i></a>
                            </td>
                            ";
                            echo "</tr>";
                    }
            ...
            <?php
                if (isset($_GET['del'])) {
                    $del = $_GET['del'];  
                    $delete = mysqli_query($con, "DELETE FROM tbl_payment WHERE id = '$del'");

                    echo "<script>alert('Successfully Deleted an Payment.');window.location.href='adminpayment.php'</script>";
                }
            ?>
            <?php 
                if (($_GET['view'])) {
                    $view = $_GET['view'];
                    $select = mysqli_query($con, "SELECT * FROM tbl_payment WHERE id = '$view'");
                    while ($row = mysqli_fetch_assoc($select)) {
                        $id = $row['id'];
    ...
    <div class="modal-dialog modal-login">
			<div class="modal-content">
				...
				<div class="modal-body">
						...
						<div class="form-group">
							<?php
                                echo "Payment ID: <br>". $id;
                            ?>		
						</div>

                        <div class="form-group">
							<?php
                                echo "Amount Paid: <br>". " P " . $amount_paid;
                            ?>		
						</div>
                        <div class="form-group">
							<?php
                                echo "GCash Number: <br>". $gcash_number;
                            ?>		
						</div>
                        <div class="form-group">
							<?php
                                echo "Reference Number: <br>". $reference_no;
                            ?>		
						</div>
                        <div class="form-group">
							<?php
                        ...
                        <div class="form-group">
							<?php
                                echo "Payment Date: <br>". $payment_date;
                            ?>		
						</div>
                        <div class="form-group">
							<?php
                                echo "Payment Time: <br>". $payment_time;
                            ?>		
						</div>
