...
<nav class="navbar navbar-expand-xl navbar-light bg-light fixed-top">
	...
	<div id="navbarCollapse" class="collapse navbar-collapse justify-content-start">
		<div class="navbar-nav">
            ...
            <a href="adminlist.php" class="nav-item nav-link active">Admins</a>
...
<div class="container-xl" data-aos="fade-up">
    <div class="table-responsive">
        <div class="table-wrapper">
            <div class="table-title">
                <div class="row">
                    <div class="col-sm-8 col-lg-9"><h1>Admin Details </h1></div>
	...
	if (isset($_POST['create'])) {
		...
		if ($fname_validation == "valid" && $lname_validation == "valid" && $gender_validation == "valid" && $email_validation == "valid" && $password_validation == "valid" && $image_validation =="valid") {
			...
			$insert_into_db = mysqli_query($con, "INSERT INTO tbl_admin (email, password, fname, lname, bday, gender, number, image) VALUES ('$email', '$hashed_password', '$fname', '$lname', '$bday', '$gender', '$number', '$image')");
			...
			if (!$insert_into_db) {
				...
				echo "<script>alert('Successfully Created an Account, Please Login using Email and Password.');window.location.href='adminlist.php'</script>";

			}
		}
		else{
			echo "<script>alert('There's Something Wrong with the input.');window.location.href='adminlist.php'</script>";
		}
...
?>
            ...
            <table class="table table-striped table-hover table-bordered">
                ...
                <?php 
                    ...
                    while ($row = mysqli_fetch_assoc($select)) {
                            ...
                            //echo "<td class='td'>". $row['image'] . '</td>';
                            echo " 
                            <td>
                                <a href='adminlist.php?view=$row[id]' class='view' title='View' data-toggle='tooltip'><i class='material-icons'>&#xE417;</i></a>
                                <a href='adminlist.php?edit=$row[id]' class='edit' title='Edit' data-toggle='tooltip'><i class='material-icons'>&#xE254;</i></a>
                                <a href='adminlist.php?del=$row[id]' class='delete' title='Delete' data-toggle='tooltip'><i class='material-icons'>&#xE872;</i></a>
                            </td>
                            ";
                            echo "</tr>";
                    }
            ...
            <?php
                if (isset($_GET['del'])) {
                    $del = $_GET['del'];  
                    $delete = mysqli_query($con, "DELETE FROM tbl_admin WHERE id = '$del'");

                    echo "<script>alert('Successfully Deleted an Account.');window.location.href='adminlist.php'</script>";
                }
            ?>
            <?php 
                if (($_GET['view'])) {
                    $view = $_GET['view'];
                    $select = mysqli_query($con, "SELECT * FROM tbl_admin WHERE id = '$view'");
                    while ($row = mysqli_fetch_assoc($select)) {
                        $fname = $row['fname'];
                        $lname = $row['lname'];
                        $id = $row['id'];
    ...
    <?php
        ...
        elseif (($_GET['edit'])) {
            $edit = $_GET['edit'];
            $select = mysqli_query($con, "SELECT * FROM tbl_admin WHERE id = '$edit'");
            while ($row = mysqli_fetch_assoc($select)) {
                $fname = $row['fname'];
                $lname = $row['lname'];
                $id = $row['id'];
    ...
    if (isset($_POST['submit'])) {
        ...
        if ($fname_validation == "valid" && $lname_validation == "valid" && $gender_validation == "valid" && $email_validation == "valid" && $password_validation == "valid" && $image_validation =="valid") {
            ...
            if (!$update) {
                ...
                echo "<script>alert('Successfully Updated an Account.');window.location.href='adminlist.php'</script>";
